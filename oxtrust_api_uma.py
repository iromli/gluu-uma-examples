...
def get_kid(alg="RS256", jwks="api-rp-keys.json"):
    kid = ""
...
def get_groups(token=""):
    req = requests.get(
        f"{get_base_url()}/identity/restv1/api/v1/groups",
        headers={
...
def main():
    ...
    if not req.ok:
        ...
        alias = get_kid()
        with open("api_rp_jks_pass") as f:
            jks_pass = f.read()

        jks_to_pkcs12("api-rp.jks", "api-rp.pkcs12", alias, jks_pass)
        pubcert, privkey = certkey_from_pkcs12("api-rp.pkcs12", "api-rp.pem", jks_pass)

        jwt = generate_jwt(alias, privkey, client_id)
        print(jwt)
        token = get_token(client_id, jwt, ticket)
        req = get_groups(token)
